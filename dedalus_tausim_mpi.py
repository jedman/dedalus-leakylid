...
nx, nz = (144, 280)  # number of points in each direction
...
model_top = 5. * Lz # lid height
if eps < 0.41:
    model_top = 5. * Lz # increases resolution near the jump 
...
for k in range(8,10):
    ...
    solver.stop_wall_time = np.inf
    solver.stop_iteration = np.inf
    B['g'] = 0.001*np.sin(m * np.pi*z/Lz)*np.cos(k* 2* np.pi* x /Lx) 
    B['g'][:,strat] = 0.
    # CFL conditions
    initial_dt = 0.8*Lz/nz
